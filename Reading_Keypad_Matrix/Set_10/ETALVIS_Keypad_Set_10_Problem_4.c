...
void loop() {
  ...
  while (1) {
    ...
    if (key) {
      ...
      if (key >= '0' && key <= '9') {
        if (num_flag == 0 && num1_flag < 2) {
          uint8_t digit = key - '0';
          num1 = num1*10 + digit;
          num1_flag++;

          digits[0] = numbers[num1/10];
          digits[1] = numbers[num1%10];
          digits[2] = 0;
          digits[3] = 0;
        } 
        else if (num_flag == 1 && num2_flag < 2) {
          uint8_t digit = key - '0';
          num2 = num2*10 + digit;
          num2_flag++;
          digits[0] = 0;
          digits[1] = 0;
          digits[2] = numbers[num2/10];
          digits[3] = numbers[num2%10];
          operation_flag = 1;
        }
      }
      // If plus is pressed set num_flag = 1 to scan num2
      else if (key == '+' && digits[0] !=0) {
        num_flag = 1;
      }
      ...
      // on seven segment display
        ...
        if(sum>=100){ // Display case when sum is 3 digit
          digits[0] = numbers[sum / 100];
          digits[1] = numbers[(sum/10)%10];
          digits[2] = numbers[sum % 10];
          digits[3] = 0;
        }
        else{ // Display case when sum is 2 digit
          digits[0] = numbers[sum / 10];
          digits[1] = numbers[sum % 10];
          digits[2] = 0;
          digits[3] = 0;
        }
