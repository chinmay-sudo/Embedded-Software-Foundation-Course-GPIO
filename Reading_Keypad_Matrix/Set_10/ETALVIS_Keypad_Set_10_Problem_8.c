/*
  Perform multiplication. If the total exceeds 9999 then display "EEEE".
*/
#include <stdint.h>
...
void loop() {
  ...
  while (1) {
    ...
    if (key) {
      ...
      // If A (*) is pressed set num_flag = 1 to scan num2
      ...
      // on seven segment display
        ...
        // update the digit values to display result
        if(result>9999){ // Display case when result is greater than 9999
          digits[0] = 0x79;
          digits[1] = 0x79;
          digits[2] = 0x79;
          digits[3] = 0x79;
        }
        else{ // Display case when result is 4 digit or lesser
          digits[0] = numbers[result / 1000];
          digits[1] = numbers[(result/100)%10];
          digits[2] = numbers[(result/10)%10];
          digits[3] = numbers[result % 10];
        }
