/*
  Perform subtraction. If the total exceeds 9999 then display "EEEE".
...
// Display fucntion to display the difference
void display_result(int difference, char *digits){
  // update the digit values to display difference
  // Display case when difference is greater than 9999
  if(difference>9999 || difference<-999){ 
    digits[0] = 0x79;
    digits[1] = 0x79;
    digits[2] = 0x79;
    digits[3] = 0x79;
    ...
    if(value < 10){
      ...
      digits[1] = 0;
      digits[2] = 0x40;
      digits[3] = numbers[value];
    }
    else if(value < 100){
      digits[0] = 0;
      digits[1] = 0x40;
      digits[2] = numbers[value/10];
      digits[3] = numbers[value%10];
    }
    else if(value < 1000){
      digits[0] = 0x40;
      digits[1] = numbers[value/100];
      digits[2] = numbers[(value/10)%10];
      digits[3] = numbers[value%10];
    }
  }
  // Display case when difference is 4 digit or lesser
  else{ 
    digits[0] = numbers[difference / 1000];
    digits[1] = numbers[(difference/100)%10];
    digits[2] = numbers[(difference/10)%10];
    digits[3] = numbers[difference % 10];
  }
...
void loop() {
  ...
  while (1) {
    ...
    if (key) {
      // If digits are pressed scan for num1 and num2
      if (key >= '0' && key <= '9') {
      ...
      // on seven segment display
        ...
        display_result(difference, digits);
