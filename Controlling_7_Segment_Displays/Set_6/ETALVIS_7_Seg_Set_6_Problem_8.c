/*
  ...
  DISPLAY odd numbers from 1 to 9, one by one with 
  delay of 1 sec and display even numbers from 8 to 0
  with delay of 1 sec
*/
...
void loop() {
  ...
  // Display odd numbers from 1 to 9
  for(int i =1;i<10;i+=2){
    PORTA = ~numbers[i]; //Inverting bits as it is common anode
    delay_sec();
  }
  // Display even numbers from 8 to 0 
  for(int i = 8;i>=0;i-=2){
    PORTA = ~numbers[i]; //Inverting bits as it is common anode
    delay_sec();
  }
